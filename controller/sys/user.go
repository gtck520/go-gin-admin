...
func (User) Info(c *gin.Context) {
	// 用户ID
	uid, isExit := c.Get(codes.USER_ID_Key)
	if !isExit {
	...
	var err error
	if userID == codes.SUPER_ADMIN_ID {
		//管理员
	...
	if len(menus) == 0 && userID == codes.SUPER_ADMIN_ID {
...
func (User) EditPwd(c *gin.Context) {
	...
	old_password = hash.Md5String(codes.MD5_PREFIX + old_password)
	new_password:=reqData["new_password"]
	if len(new_password)<6 || len(new_password)>20 {
		common.ResFail(c, "密码长度在 6 到 20 个字符")
		return
	}
	new_password = hash.Md5String(codes.MD5_PREFIX + new_password)
	where := sys.Admins{}
	where.ID = userID
	modelOld := sys.Admins{}
